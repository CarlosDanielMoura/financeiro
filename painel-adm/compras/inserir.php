...
$total_venda = 0;
$query_con = $pdo->query("SELECT * FROM itens_compra WHERE id_compra = 0 and 
...
if($status == 'Concluída'){
	...
	if($parcelas > 1){
		$query = $pdo->query("UPDATE contas_pagar set cliente = '$cliente', saida = '$lancamento',
         documento = '$pagamento', plano_conta = 'Compra de Produtos - Empresa', 
         frequencia = 'Uma Vez', usuario_lanc = '$id_usuario', status = 'Pendente', 
         data_recor = curDate(), id_compra = '$id_ult_registro' WHERE id_compra = '-1' 
         and usuario_lanc = '$id_usuario'");
	}else{
		$query = $pdo->query("INSERT INTO contas_pagar set descricao = '$descricao_conta', 
        cliente = '$cliente', saida = '$lancamento', documento = '$pagamento', 
        plano_conta = 'Compra de Produtos - Empresa', data_emissao = curDate(), 
        vencimento = '$data', frequencia = 'Uma Vez', valor = '$subtotal', 
        usuario_lanc = '$id_usuario', status = 'Pendente', data_recor = curDate(), 
        id_compra = '$id_ult_registro', arquivo = 'sem-foto.jpg'");
	}
}



$query_con = $pdo->query("SELECT * FROM itens_compra WHERE id_compra = 0
...
if($total_reg > 0){ 
	...
	foreach ($res[$i] as $key => $value){	}
		...
		$pdo->query("UPDATE itens_compra set id_compra = '$id_ult_registro' where 
        id_compra = 0 and usuario = '$id_usuario'");
	}
